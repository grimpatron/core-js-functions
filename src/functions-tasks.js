...
function logger(func, logFunc) {
  return (...args) => {
    const argsAsAStr = args.map((arg) => JSON.stringify(arg)).join(',');
    logFunc(`${func.name}(${argsAsAStr}) starts`);

    const result = func.apply(this, args);

    logFunc(`${func.name}(${argsAsAStr}) ends`);
    return result;
  };
}
